...
description: Manage the shared Taskboard with taskctl from Claude Code, Codex, or pi. Use for taskboard issue IDs, claiming work, status sync, and comments—not for unrelated product docs.
---
...
- Use `taskctl` from `PATH`. It talks to `CODEX_TASKBOARD_URL`, default `http://127.0.0.1:47823`. On the user's machines that port is an SSH tunnel to the one shared board on the Mac mini, so every machine sees the same issues. Do not start another Taskboard server, switch ports, or write to a different board.
- If `taskctl` exits with code `3` (service unavailable), stop and report that the board is unreachable.
...
- Only the agent conversation doing the task writes to the board. If another agent started you as a one-shot helper—a `/peerreview` peer or verdict round, an image render, or any other `codex exec` subprocess—never run `taskctl` write commands; return your result to the caller.
- Labels reserve work for one agent: `for-claude`, `for-codex`, `for-pi`. An issue with none of them may be claimed by any agent. Never claim an issue reserved for another agent; leave it and report it.
- `taskctl` records the agent, host, and conversation of every write automatically.
...
5. Execute only the requested work in the issue's branch or worktree when one is bound. If none is bound, record the branch you work on with `issue update --git-branch BRANCH` right after the claim. In a claim comment, name the repository path and, for GitHub-backed work, the GitHub issue or PR URL, so another session or machine can resume it.
...
- `taskctl` reads the current conversation from `CODEX_THREAD_ID`, `CLAUDE_CODE_SESSION_ID`, or `PI_SESSION_ID`. Pass `--thread-id` only when none of them is set.
- An `in_progress` issue belongs to the conversation that claimed it (its `threadId`). Never take over an issue claimed by another conversation; ask the user instead.
- Do not pass `--binding-*` options; they belong to the Codex desktop integration. If an issue already has a complete `threadBinding`, preserve its saved values on every status write.
